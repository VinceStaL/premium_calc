...
import { uploadProductRates, UploadProgress } from '../lib/productRateService';
...
const STATE_CODES = ['A', 'N', 'Q', 'S', 'T', 'V', 'W', 'X'];
const YES_NO_OPTIONS = ['Y', 'N'];
const HEALTH_CATEGORIES = [2, 3, 5];
...
export function ProductRateUpload({ onBack }: { onBack: () => void }) {
  ...
  function validateRow(row: any, rowIndex: number): { isValid: boolean; errors: string[] } {
    ...
    if (!row.StateCode) errors.push(`Row ${rowIndex + 1}: State Code is required`);
    if (row.RateCode === undefined || row.RateCode === null) errors.push(`Row ${rowIndex + 1}: Rate Code is required`);
    ...
    if (row.HealthCategory === undefined || row.HealthCategory === null) errors.push(`Row ${rowIndex + 1}: Health Category is required`);
    if (row.BaseRate === undefined || row.BaseRate === null) errors.push(`Row ${rowIndex + 1}: Base Rate is required`);
    if (!row.LHCApplicable) errors.push(`Row ${rowIndex + 1}: LHC Applicable is required`);
    if (!row.RebateApplicable) errors.push(`Row ${rowIndex + 1}: Rebate Applicable is required`);

    if (row.ProductCode && row.ProductCode.length !== 3) {
      errors.push(`Row ${rowIndex + 1}: Product Code must be exactly 3 characters`);
    }

    if (row.StateCode && !STATE_CODES.includes(row.StateCode)) {
      errors.push(`Row ${rowIndex + 1}: Invalid State Code`);
    }

    if (row.RateCode !== undefined && (row.RateCode < 0 || row.RateCode > 999)) {
      errors.push(`Row ${rowIndex + 1}: Rate Code must be between 0 and 999`);
    }
    ...
    if (row.HealthCategory && !HEALTH_CATEGORIES.includes(Number(row.HealthCategory))) {
      errors.push(`Row ${rowIndex + 1}: Health Category must be 2, 3, or 5`);
    }

    if (row.BaseRate !== undefined && row.BaseRate < 0) {
      errors.push(`Row ${rowIndex + 1}: Base Rate must be greater than or equal to zero`);
    }

    if (row.LHCApplicable && !YES_NO_OPTIONS.includes(row.LHCApplicable)) {
      errors.push(`Row ${rowIndex + 1}: LHC Applicable must be Y or N`);
    }
    if (row.RebateApplicable && !YES_NO_OPTIONS.includes(row.RebateApplicable)) {
      errors.push(`Row ${rowIndex + 1}: Rebate Applicable must be Y or N`);
    }
  ...
  async function handleStartUpload() {
    ...
    try {
      ...
      rows.forEach((row: any, index) => {
        ...
        if (row.BaseRate !== undefined && row.BaseRate !== null) {
          row.BaseRate = Number(Number(row.BaseRate).toFixed(2));
        }

        const { isValid, errors } = validateRow(row, index);
        if (isValid) {
          validRows.push(row as ProductRate);
        } else {
      ...
      if (validRows.length > 0) {
        try {
          await uploadProductRates(validRows, session.user.email, deleteAll, setUploadProgress, newController.signal);
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        >
          ...
          Back to Product Rate Master
        </button>
      </div>

      <div className="bg-white rounded-lg shadow-md p-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Upload Product Rate Master</h1>
          ...
          {showInstructions && (
            <div className="mt-4 space-y-4">
              ...
              <ul className="list-disc list-inside space-y-2 text-gray-600">
                <li>Upload an Excel file (.xlsx) containing product rate data</li>
                <li>The file should have the following columns: ProductCode, StateCode, RateCode, DateOn, DateOff, HealthCategory, BaseRate, LHCApplicable, RebateApplicable</li>
                <li>All fields are required</li>
                <li>Product Code must be exactly 3 characters</li>
                <li>State Code must be one of: {STATE_CODES.join(', ')}</li>
                <li>Rate Code must be between 0 and 999</li>
                <li>Health Category must be one of: {HEALTH_CATEGORIES.join(', ')}</li>
                <li>Base Rate must be greater than or equal to zero</li>
                <li>LHC Applicable and Rebate Applicable must be 'Y' or 'N'</li>
