...
import { uploadScaleFactors, UploadProgress } from '../lib/productRateService';
...
export function ScaleFactorsUpload({ onBack }: { onBack: () => void }) {
  ...
  function validateRow(row: any, rowIndex: number): { isValid: boolean; errors: string[] } {
    ...
    if (row.ScaleFactor === undefined || row.ScaleFactor === null) errors.push(`Row ${rowIndex + 1}: Scale Factor is required`);

    if (row.ProductCode && row.ProductCode.length !==3) {
      errors.push(`Row ${rowIndex + 1}: Product Code must be exactly 3 characters`);
    }

    if (row.ScaleCode && !SCALE_CODES.includes(row.ScaleCode)) {
    ...
    if (row.ScaleFactor !== undefined && (row.ScaleFactor < 0 || row.ScaleFactor > 10)) {
      errors.push(`Row ${rowIndex + 1}: Scale Factor must be between 0 and 10`);
    }
  ...
  async function handleStartUpload() {
    ...
    try {
      ...
      rows.forEach((row: any, index) => {
        ...
        if (row.ScaleFactor !== undefined && row.ScaleFactor !== null) {
          row.ScaleFactor = Number(Number(row.ScaleFactor).toFixed(3));
        }

        const { isValid, errors } = validateRow(row, index);
        if (isValid) {
          validRows.push(row as ScaleFactor);
        } else {
      ...
      if (validRows.length > 0) {
        try {
          await uploadScaleFactors(validRows, session.user.email, deleteAll, setUploadProgress, newController.signal);
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        >
          ...
          Back to Scale Factors
        </button>
      </div>

      <div className="bg-white rounded-lg shadow-md p-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Upload Scale Factors</h1>
          ...
          {showInstructions && (
            <div className="mt-4 space-y-4">
              ...
              <ul className="list-disc list-inside space-y-2 text-gray-600">
                <li>Upload an Excel file (.xlsx) containing scale factor data</li>
                <li>The file should have the following columns: ProductCode, ScaleCode, DateOn, DateOff, ScaleFactor</li>
                <li>All fields are required</li>
                <li>Product Code must be exactly 3 characters</li>
                <li>Scale Code must be one of: {SCALE_CODES.join(', ')}</li>
                <li>Scale Factor must be between 0 and 10 with up to 3 decimal places</li>
