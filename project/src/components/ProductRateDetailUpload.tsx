...
import { uploadProductRateDetails, UploadProgress } from '../lib/productRateService';
import { SCALE_CODES } from '../lib/constants';

interface ProductRateDetail {
  ProductCode: string;
  StateCode: string;
  RateCode: number;
  ScaleCode: string;
...
export function ProductRateDetailUpload({ onBack }: { onBack: () => void }) {
  ...
  function validateRow(row: any, rowIndex: number): { isValid: boolean; errors: string[] } {
    ...
    if (!row.ScaleCode) errors.push(`Row ${rowIndex + 1}: Scale Code is required`);
    ...
    if (row.ScaleCode && !SCALE_CODES.find(s => s.code === row.ScaleCode)) {
      errors.push(`Row ${rowIndex + 1}: Invalid Scale Code`);
    }
    ...
    rates.forEach(rate => {
      if (row[rate] !== undefined && row[rate] !== null && row[rate] < 0) {
        errors.push(`Row ${rowIndex + 1}: ${rate.replace('Rate', '')} Rate must be greater than or equal to zero`);
      }
    });
  ...
  async function handleStartUpload() {
    ...
    try {
      ...
      const validRows: ProductRateDetail[] = [];
      ...
      rows.forEach((row: any, index) => {
        ...
        const rates = ['WeeklyRate', 'MonthlyRate', 'QuarterlyRate', 'HalfYearlyRate', 'YearlyRate'];
        rates.forEach(rate => {
          if (row[rate] !== undefined && row[rate] !== null) {
            row[rate] = Number(Number(row[rate]).toFixed(2));
          }
        });

        const { isValid, errors } = validateRow(row, index);
        if (isValid) {
          validRows.push(row as ProductRateDetail);
        } else {
      ...
      if (validRows.length > 0) {
        try {
          await uploadProductRateDetails(validRows, session.user.email, deleteAll, setUploadProgress, newController.signal);
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        >
          ...
          Back to Product Rate Detail
        </button>
      </div>

      <div className="bg-white rounded-lg shadow-md p-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Upload Product Rate Detail</h1>
          ...
          {showInstructions && (
            <div className="mt-4 space-y-4">
              ...
              <ul className="list-disc list-inside space-y-2 text-gray-600">
                <li>Upload an Excel file (.xlsx) containing product rate detail data</li>
                <li>The file should have the following columns: ProductCode, StateCode, RateCode, ScaleCode, DateOn, DateOff, WeeklyRate, MonthlyRate, QuarterlyRate, HalfYearlyRate, YearlyRate</li>
                <li>Required fields: ProductCode, StateCode, RateCode, ScaleCode, DateOn, DateOff</li>
                <li>Product Code must be exactly 3 characters</li>
                <li>Scale Code must be one of: {SCALE_CODES.map(scale => `${scale.code} (${scale.name})`).join(', ')}</li>
                <li>All rate fields must be greater than or equal to zero</li>
